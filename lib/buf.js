...
function Buf(){
	...
	self.fil=function (value,length){ if(!bufLen) return self; length = length||(bufLen-pos); bufU8.fill(value,pos,pos+length); pos +=length; return self;};		
	//value : Buf or Buf's Array
	self.set=function (value) {
		if(!bufLen) return;
		var pos = 0;
		var len = 0;
		if(value instanceof Buf){
			len = value.length;
...
Buf.slice = function(buf, start, end){
	if(end === null||end === undefined)
		end = buf.length;
	return new Buf(buf.raw, buf.rawOffset + start, end-start);
}
//list : [Buf, Buf, ...], totalLength : sum(Buf.length)
